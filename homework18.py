...
class Car:
    ...
    def __del__(self):
        ...
        with open("car_info.csv", "a") as csv_file:
            csv_file.write('dsadsad')
            # writer = csv.writer(csv_file)
            # writer.writerow([self.brand, self.producer, "scrapped"])

class Lorry(Car):
    def __init__(self, producer, brand, year_of_release=2020, fuel_consumption: float = 0.0, cargo_capacity: float = 0.0):
        super().__init__(producer, brand, year_of_release, fuel_consumption)
        self.cargo_capacity = cargo_capacity

class SportsCar(Car):
    def __init__(self, producer, brand, year_of_release=2020, fuel_consumption: float = 0.0, max_speed: float = 0.0, price: float = 0.0):
        super().__init__(producer, brand, year_of_release, fuel_consumption)
